...
export default class celestialBody
{
    constructor(name, geometry = null, texture = null, semiMajorAxis = 0, semiMinorAxis = 0, period = 0)
    {
        ...
        this.hideAxesHelper();
        this.makeTrace();
        this.makeOrbit(0, 0, 0);
    }

    makeOrbit(centerx, centery, centerz, semiMajorAxis = this.semiMajorAxis, semiMinorAxis = this.semiMinorAxis, pointsCount = this.orbitPointsCount, material = null)
    {
        this.semiMajorAxis = semiMajorAxis;
        this.semiMinorAxis = semiMinorAxis;
        ...
        for(let i = 0; i <= pointsCount; ++i)
        {
            ...
            const x = semiMajorAxis * Math.cos(theta) + centerx;
            const z = semiMinorAxis * Math.sin(theta) + centerz;
    ...
    update(delta, simulationSpeed)
    {
        if(!this.fixed)
        {
            ...
            if(this.trace != null)
            {
                ...
                if(distance(x, z, traceVertices[traceVertices.length-3], traceVertices[traceVertices.length-1]) >= 0.1)
                {
