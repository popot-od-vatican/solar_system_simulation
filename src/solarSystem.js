...
const MERCURY_MINOR_AXIS = 59000000/toKilometers / orbitScale;
...
const EARTH_RADIUS = 6378/planetScale;
...
const MARS_RADIUS = 3396/planetScale;
...
const JUPITER_RADIUS = 71492/planetScale;
...
const SATURN_RADIUS = 60268/planetScale;
...
const URANUS_RADIUS = 25559/planetScale;
...
const NEPTUNE_RADIUS = 24764 / planetScale;
...
const MOON_RADIUS = 1737.5 / planetScale;
const MOON_MAJOR_AXIS = 384400/toKilometers / satelliteOrbitScale;
const MOON_MINOR_AXIS = 405500/toKilometers / satelliteOrbitScale;
const MOON_PERIOD = 27.3;
const MOON_ROTATION = (2 * Math.PI) / (29.5*24*60*60);

const TITAN_RADIUS = 2574.7 / planetScale;
const TITAN_MAJOR_AXIS = 1258000 / toKilometers / satelliteOrbitScale * 4;
const TITAN_MINOR_AXIS = 1186000 / toKilometers / satelliteOrbitScale * 4;
const TITAN_PERIOD = 15.94;
const TITAN_ROTATION = (2 * Math.PI) / (TITAN_PERIOD*24*60*60);

const GANYMEDE_RADIUS = 2631 / planetScale;
const GANYMEDE_MAJOR_AXIS = 1072000 / toKilometers / satelliteOrbitScale * 4;
const GANYMEDE_MINOR_AXIS = 1068000 / toKilometers / satelliteOrbitScale * 4;
const GANYMEDE_PERIOD = 7.2;
const GANYMEDE_ROTATION = (2 * Math.PI) / (7.2*24*60*60);

const EUROPA_RADIUS = 1561 / planetScale;
const EUROPA_MAJOR_AXIS = 678000 / toKilometers / satelliteOrbitScale * 4;
const EUROPA_MINOR_AXIS = 664000 / toKilometers / satelliteOrbitScale * 4;
...
class solarSystem
{
    ...
    addOrbitLinesToScene(scene)
    {
        for(let i = 0; i < this.planets.length; ++i) {
            this.planets[i].addOrbitToScene(scene);
        }
    }

    addOrbitTracesToScene(scene)
    {
        for(let i = 0; i < this.planets.length; ++i) {
            this.planets[i].addTraceToScene(scene);
        }
    }
...
function initSolarSystem()
{
    ...
    mercury.setOrbitPointsCount(146);
    mercury.setTracePointsCount(90);
    mercury.makeOrbit();
    mercury.makeTrace();

    venus.setOrbitPointsCount(178);
    venus.setTracePointsCount(90);
    venus.makeOrbit();
    venus.makeTrace();

    earth.setOrbitPointsCount(306);
    earth.setTracePointsCount(90);
    earth.makeOrbit();
    earth.makeTrace();

    mars.setOrbitPointsCount(362);
    mars.setTracePointsCount(90);
    mars.makeOrbit();
    mars.makeTrace();

    jupiter.setOrbitPointsCount(462);
    jupiter.setTracePointsCount(210);
    jupiter.makeOrbit();
    jupiter.makeTrace();

    saturn.setOrbitPointsCount(500);
    saturn.setTracePointsCount(330);
    saturn.makeOrbit();
    saturn.makeTrace();

    uranus.setOrbitPointsCount(540);
    uranus.setTracePointsCount(360);
    uranus.makeOrbit();
    uranus.makeTrace();

    neptune.setOrbitPointsCount(562);
    neptune.setTracePointsCount(410);
    neptune.makeOrbit();
    neptune.makeTrace();

    moon.setOrbitPointsCount(128);
    moon.setTracePointsCount(60);
    moon.makeOrbit();
    moon.makeTrace();

    titan.setOrbitPointsCount(128);
    titan.setTracePointsCount(60);
    titan.makeOrbit();
    titan.makeTrace();

    ganymede.setOrbitPointsCount(128);
    ganymede.setTracePointsCount(50);
    ganymede.makeOrbit();
    ganymede.makeTrace();

    europa.setOrbitPointsCount(128);
    europa.setTracePointsCount(50);
    europa.makeOrbit();
    europa.makeTrace();
    ...
    jupiter.setOrbitColor('purple');
    jupiter.setOrbitTraceColor('purple');
    jupiter.setLabelColor('cyan');
    saturn.setOrbitColor('brown');
    saturn.setOrbitTraceColor('brown');
    saturn.setLabelColor('grey');
    uranus.setOrbitColor('white');
    uranus.setOrbitTraceColor('white');
    uranus.setLabelColor('aqua');
    neptune.setOrbitColor('aqua');
    neptune.setOrbitTraceColor('aqua');
    ...
    titan.setOrbitColor('lime');
    titan.setOrbitTraceColor('lime');
